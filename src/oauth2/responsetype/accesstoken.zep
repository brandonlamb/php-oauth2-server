...
class AccessToken implements AccessTokenInterface
{
    ...
    public function __construct(<AccessTokenStorageInterface> tokenStorage, <RefreshTokenInterface> refreshStorage = null, array config = [])
    {
        ...
        let this->refreshStorage = refreshStorage;
    ...
    public function getAuthorizeResponse(var params, string! user_id = null) -> array
    {
        ...
        let result["fragment"] = this->createAccessToken(params["client_id"], user_id, params["scope"], includeRefreshToken);

        if isset params["state"] {
            let result["fragment"]["state"] = params["state"];
        }
    ...
    public function createAccessToken(string! client_id, string! user_id, string! scope = null, boolean includeRefreshToken = true) -> string
    {
        ...
        if includeRefreshToken && this->refreshStorage {
            let token["refresh_token"] = this->generateRefreshToken();
            let expires = 0;
    ...
    protected function generateAccessToken() -> string
    {
        ...
        if function_exists("mcrypt_create_iv") {
            let randomData = mcrypt_create_iv(20, MCRYPT_DEV_URANDOM);
        ...
        if function_exists("openssl_random_pseudo_bytes") {
            let randomData = openssl_random_pseudo_bytes(20);
            if randomData !== false && strlen(randomData) === 20 {
                return bin2hex(randomData);
            }
        }
        ...
        if file_exists("/dev/urandom") {
            ...
            let randomData = file_get_contents("/dev/urandom", false, null, 0, 20);
            if randomData !== false && strlen(randomData) === 20 {
                return bin2hex(randomData);
            }
        }
