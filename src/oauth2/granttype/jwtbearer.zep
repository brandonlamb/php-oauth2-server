...
class JwtBearer implements GrantTypeInterface, ClientAssertionTypeInterface
{
     ...
     */
    public function __construct(<JwtBearerInterface> storage, string! audience, <EncryptionInterface> jwtUtil = null)
    {
        let this->storage = storage;
        let this->audience = audience;

        if typeof jwtUtil == "null" {
            let jwtUtil = new Jwt();
        }

        let this->jwtUtil = jwtUtil;
    }
    ...
    {
        var undecodedJWT, jwt, notBefore, jti, key;
        ...
        let undecodedJWT = request->request("assertion");

        // Decode the JWT
        let jwt = this->jwtUtil->decode(request->request("assertion"), null, false);
        ...
        if isset jwt["jti"] {
            let jti = this->storage->getJti(jwt["iss"], jwt["sub"], jwt["aud"], jwt["exp"], jwt["jti"]);
        ...
        let key = this->storage->getClientKey(jwt["iss"], jwt["sub"]);
        if !key {
